...
# --- CONFIGURAÇÃO VISUAL ---
st.set_page_config(page_title="Radar VIP - Modo Automático", layout="wide")
...
st.title("🎯 MASTER RADAR VIP: FUTEBOL ELITE")
status = st.empty()

# --- FUNÇÃO PARA PEGAR DADOS DO COFRE (SECRETS) ---
...
with st.sidebar:
    ...
    # Lista de APIs cadastradas no cofre
    opcao_api = st.selectbox("Escolher conta da API:", ["Conta 1", "Conta 2", "Conta 3"])
    
    # Tenta puxar do cofre, se não existir, fica vazio para digitar
    if opcao_api == "Conta 1":
        api_key_padrao = get_secret("api_key_1")
    elif opcao_api == "Conta 2":
        api_key_padrao = get_secret("api_key_2")
    else:
        api_key_padrao = get_secret("api_key_3")
        
    api_key = st.text_input(f"Chave API ({opcao_api}):", value=api_key_padrao, type="password")
    ...
    # Puxa dados do Telegram direto do cofre
...
def get_ligas_futebol(chave):
    ...
    try:
        ...
        # Filtro de Ligas: Brasil Total + Continentais + Outros (Apenas 1ª Div)
        ...
        for liga in res:
            ...
            elif not any(b in k for b in bloqueio):
                selecionadas.append(liga['key'])
        return selecionadas
    except: return []
        ...
        try:
            ...
            for jogo in res:
                ...
                if min_f <= o_fav <= max_f and o_zeb >= min_z:
                    ...
                    jogos.append({"⏰ Hora": h_br, "🏆 Liga": jogo["sport_title"], "🛡️ Fav": fav, "🦓 Zeb": zeb, "📈 Odd F": o_fav, "📉 Odd Z": o_zeb, "🏦 Casa": book['title'], "📍 Local": loc})
        except: pass
